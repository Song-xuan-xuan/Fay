...
describe('audioControls', () => {
  it('toggles one audio flag without mutating the current config', () => {
    const current = { mic: true, speaker: false, micMode: 'wake' as const };
    const next = toggleAudioFlag(current, 'speaker');

    expect(next).toEqual({ mic: true, speaker: true, micMode: 'wake' });
    expect(current).toEqual({ mic: true, speaker: false, micMode: 'wake' });
  });
  ...
  it('includes the selected microphone mode in the record config', () => {
    expect(buildAudioConfigPatch({ mic: true, micMode: 'continuous' })).toEqual({
      source: { record: { enabled: true, mode: 'continuous' } },
    });
  });
});
