...
def _build_execution_next_step_messages(state: ExecutionState) -> list:
    ...
    system_content = (
        ...
        '\n搜索策略：\n'
        '- query_yueshen 用于 RAG 原始资料/结构化数据检索，尤其适合景点编号、参数、面积、位置、开放时间、门票等事实型问题\n'
        '- kb_search 用于课程包检索，尤其适合游览路线、兴趣推荐、讲解重点、章节讲稿等讲解型问题\n'
        '- 如果已调用的知识工具结果不够相关，优先切换到另一类知识工具，而不是反复用同一工具换关键词\n'
        '- 可以用 kb_list_sources 查看课程列表，找到最相关的课程后用 source_id 定向搜索\n'
        '- 优先从标题或来源最匹配的资料中获取信息\n'
        f'\n可用工具：\n{tools_text}\n'
    )
    ...
    if state.unverified_response and not state.tool_results:
        verify_block = (
            ...
            "这一轮的任务是：\n"
            "1. 从可用工具中挑一个合适的来核实上面回答的事实性内容（如知识库搜索等）\n"
