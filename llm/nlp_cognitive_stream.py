...
from utils.multimodal_image import image_ref_to_llm_url
...
from llm.rag_citations import append_inline_citations, build_citation_section
...
def _normalize_tool_output(result: Any) -> str:
    ...
    if segments:
        ...
        if cleaned:
            return append_inline_citations("\n".join(cleaned), result)

    # 如果提取失败，尝试返回简化的 JSON
    try:
        text = json.dumps(result, ensure_ascii=False, default=lambda o: getattr(o, "__dict__", str(o)))
        return append_inline_citations(text, result)
    except TypeError:
        return append_inline_citations(str(result), result)
...
def _build_dialogue_messages(
    ...
    conversation 格式：
    [
        {"role": "user", "content": "文本", "images": ["url1", "url2"]},
    ...
    for msg in conversation:
        role = str(msg.get("role", "")).lower()
        content = msg.get("content", "")
        images = msg.get("images", [])  # 新增：获取图片列表

        content = _remove_prestart_from_text(content)
        content = _remove_think_from_text(content)

        if not content or not content.strip():
            if not images:  # 没有文本也没有图片，跳过
                continue
            content = ""  # 仅有图片的情况
        ...
        if role in ("user", "human", "用户"):
            ...
            # 构建多模态内容
            if images:
                util.log(1, f"构建多模态消息：文本长度={len(cleaned)}, 图片数量={len(images)}, 图片URL={images}")
                # OpenAI 多模态格式
                content_parts = []
                if cleaned:
                    text_content = f"{display_name}：{cleaned}" if need_user_label else cleaned
                    content_parts.append({"type": "text", "text": text_content})

                for img_url in images:
                    util.log(1, f"处理图片：{img_url}")
                    llm_url = image_ref_to_llm_url(img_url)
                    if llm_url:
                        util.log(1, f"图片已转换为模型可读地址，类型={'data_url' if llm_url.startswith('data:image/') else 'url'}")
                        content_parts.append({
                            "type": "image_url",
                            "image_url": {"url": llm_url}
                        })
                    else:
                        util.log(1, f"跳过不可读取的图片：{img_url}")

                # LangChain 的 HumanMessage 支持多模态内容
                if content_parts:
                    util.log(1, f"创建多模态消息，部分数量={len(content_parts)}")
                    messages.append(HumanMessage(content=content_parts))
                else:
                    util.log(1, f"警告：图片处理后 content_parts 为空")
            else:
                # 纯文本消息
    ...
    for m in messages:
        if normalized and type(normalized[-1]) is type(m):
            # 合并连续同角色消息
            # 注意：多模态消息不能简单字符串拼接
            if isinstance(m.content, str) and isinstance(normalized[-1].content, str):
...
"""
        ...
        citation_section = build_citation_section(finished_exec_state.tool_results or [])
        if citation_section:
            citation_text = f"\n\n{citation_section}"
            accumulated += citation_text
...
"""
    ...
    if isolate_by_user:
        ...
        def append_to_buffer(role: str, text_value: str, msg_images_str: str = None) -> None:
            if not text_value and not msg_images_str:
                return
            # 清理 think 标签内容，避免泄漏到 LLM 输入
            text_value = _remove_think_from_text(text_value)
            if not text_value or not text_value.strip():
                if not msg_images_str:
                    return
                text_value = ""  # 纯图片消息
            ...
            if msg_images_str:
                try:
                    import json
                    msg_images_list = json.loads(msg_images_str) if isinstance(msg_images_str, str) else msg_images_str
                except:
                    pass

            messages_buffer.append({"role": role, "content": text_value, "images": msg_images_list})
            if len(messages_buffer) > 20:
                del messages_buffer[:-20]

        for record in history_records:
            msg_type, msg_text, msg_images = record
            role = 'assistant'
            if msg_type and msg_type.lower() in ('member', 'user'):
                role = 'user'
            append_to_buffer(role, msg_text, msg_images)
        ...
        if (
            ...
            messages_buffer.append({"role": "user", "content": content, "images": images})
    else:
        # 不隔离：按独立消息存储，保留用户名信息
        def append_to_buffer_multi(role: str, text_value: str, msg_username: str = "", msg_images_str: str = None) -> None:
            if not text_value and not msg_images_str:
                return
            # 清理 think 标签内容，避免泄漏到 LLM 输入
            text_value = _remove_think_from_text(text_value)
            ...
            messages_buffer.append({"role": role, "content": text_value, "username": msg_username, "images": msg_images_list})
            if len(messages_buffer) > 20:
                del messages_buffer[:-20]
        ...
        for record in history_records:
            msg_type, msg_text, msg_username, msg_images = record
            if not msg_text and not msg_images:
                continue
            if msg_type and msg_type.lower() in ('member', 'user'):
                append_to_buffer_multi("user", msg_text, msg_username, msg_images)
            else:
                append_to_buffer_multi("assistant", msg_text, "", None)
        ...
        if (
            ...
            messages_buffer.append({"role": "user", "content": content, "username": username, "images": images})
    ...
    def append_citations_to_stream(tool_results) -> None:
        nonlocal accumulated_text, full_response_text
        citation_section = build_citation_section(tool_results or [])
        if not citation_section:
            return
        citation_text = f"\n\n{citation_section}"
        accumulated_text += citation_text
        full_response_text += citation_text
    ...
    def run_workflow(tool_registry: Dict[str, WorkflowToolSpec]) -> bool:
        ...
        if final_stream_done:
            append_citations_to_stream(final_state.get("tool_results") or [])
...
"""
        ...
        if not sm.should_stop_generation(username, conversation_id=conversation_id):
            append_citations_to_stream(finished_state.tool_results)
