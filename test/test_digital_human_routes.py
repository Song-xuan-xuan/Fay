...
class DigitalHumanRoutesTest(TempProjectMixin, unittest.TestCase):
    ...
    def _write_live2d_samples(self):
        for model in ("Haru", "Hiyori", "Mao", "Natori", "miku_sample_t04"):
    ...
    def test_public_active_digital_human_requires_no_auth_and_filters_private_fields(self):
        client = self._client()

        response = client.get("/api/public/digital-human")

        self.assertEqual(200, response.status_code, response.get_data(as_text=True))
        payload = response.get_json()
        self.assertTrue(payload["success"])
        self.assertEqual(
            {"name", "type", "render_url", "cover_url"},
            set(payload["digital_human"]),
        )
        self.assertEqual("Fay", payload["digital_human"]["name"])
    ...
    def test_imports_live2d_resource_models(self):
        ...
        self.assertNotIn("live2d_natori", imported_ids)
        self.assertNotIn("live2d_miku_sample_t04", imported_ids)

    def test_imports_live2d_resource_models_uses_configured_render_base_url(self):
        with open("config.json", "r", encoding="utf-8") as file:
            config = json.load(file)
        config["live2d"] = {"render_base_url": "https://example.com/live2d"}
        with open("config.json", "w", encoding="utf-8") as file:
        ...
        self.assertEqual(200, imported.status_code, imported.get_data(as_text=True))
        payload = imported.get_json()
        by_id = {item["id"]: item for item in payload["imported"]}
        self.assertEqual("https://example.com/live2d?model=Haru", by_id["live2d_haru"]["render_url"])
