...
MCP_BIND_HOST = os.getenv('FAY_MCP_HOST', '127.0.0.1')
...
def _serialize_for_json(obj: Any):
    """递归转换 MCP 返回对象，避免 Flask jsonify 序列化失败。"""
...
def start():
    ...
    # 输出启动信息
    util.log(1, f"MCP服务已启动在 {MCP_BIND_HOST}:5010")
...
if __name__ == '__main__':
    ...
    app.run(host=MCP_BIND_HOST, port=5010, debug=True)
